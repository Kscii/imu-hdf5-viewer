...
export class IncrementalSha256 {
  ...
  update(input: Uint8Array) {
    if (this.complete) throw new Error("SHA-256 is already finalized");
    ...
    while (cursor + 64 <= input.length) {
      this.transform(input.subarray(cursor, cursor + 64));
      cursor += 64;
    }
    if (cursor < input.length) {
      this.buffer.set(input.subarray(cursor), 0);
      this.buffered = input.length - cursor;
    }
    return this;
  }
