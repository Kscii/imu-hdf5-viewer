...
Small browser fixtures are generated by `scripts/create-test-fixtures.py`. CI opens
both profiles in Chromium, Firefox, and WebKit. A release still requires the current
full-size delivery to pass `scripts/accept-client-h5.mjs` and observed Windows and
macOS acceptance before the public download is switched.
